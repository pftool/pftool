...
class OLD_MARFS_Path : public Path
{
protected:
   friend class Pool<OLD_MARFS_Path>;
   ...
   void set_err_string(int err_no, IOBuf *iob)
   {
      ...
      if (iob && iob->result)
      {
         _err_str += (std::string(", curl: '") + iob->result + "'");
      }
   }
...
public:
   ...
   virtual bool identical(Path *p)
   {
      OLD_MARFS_Path *p2 = dynamic_cast<OLD_MARFS_Path *>(p);
      return (p2 &&
              p2->exists() &&
              (st().st_ino == p2->st().st_ino));
   }
   ...
   virtual bool open(int flags, mode_t mode)
   {
      ...
      if ((flags & (O_WRONLY | O_RDWR)) && (flags & O_CREAT))
      {
         ...
         if (!exists() && marfs_mknod(marPath, mode, 0))
         {
            set_err_string(errno, NULL);
            if (errno != EEXIST)
            {
   ...
   virtual bool close()
   {
      ...
      if (!packed)
      {
         ...
         OLD_MARFS_Path::send_timing_data(whichFh);
      }
   ...
   virtual ssize_t write(char *buf, size_t count, off_t offset)
   {
      ...
      if (bytes == (ssize_t)-1)
      {
         ...
         { // 500 Internal Server Error
            ...
            if (AWS4C_OK(iob))
            {
               const size_t expected_size = (whichFh->write_status.user_req + MARFS_REC_UNI_SIZE);
